...
/**
 * 获取指定节点的所有子节点key（包括深层嵌套的子节点）
 * @param {Array<Object>} tree - 树结构
 * @param {string|number} parentKey - 父节点key
 * @param {string} keyField - key字段名，默认为 'key'
 * @returns {Array<string|number>} 所有子节点的key数组
 */
function getAllChildKeys(tree, parentKey, keyField = 'key') {
    const childKeys = [];
    
    const findChildren = (nodes, targetKey) => {
        for (const node of nodes) {
            if (node[keyField] === targetKey && node.children) {
                // 递归收集所有子节点的key
                const collectKeys = (children) => {
                    children.forEach(child => {
                        childKeys.push(child[keyField]);
 ...
 */
function findNodeByKey(tree, key, keyField = 'key') {
    return findNode(tree, node => node[keyField] === key);
}
 ...
 * @param {Array<Object>} tree - 树结构
 * @param {string|number} parentKey - 父节点key
 * @param {Array<Object>} children - 要添加的子节点数组
 * @param {string} keyField - key字段名，默认为 'key'
 * @returns {Array<Object>} 更新后的树结构
 */
function addChildrenToNode(tree, parentKey, children, keyField = 'key') {
    return tree.map(node => {
        if (node[keyField] === parentKey) {
            return {
                ...node,
        ...
        if (node.children && node.children.length > 0) {
            return {
                ...node,
                children: addChildrenToNode(node.children, parentKey, children, keyField)
            };
        }
        return node;
    });
